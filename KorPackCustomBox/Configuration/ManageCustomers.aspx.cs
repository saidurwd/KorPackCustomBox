...
public partial class UA_ManageRole : System.Web.UI.Page
{
    ...
    protected void mSave()
    {
        ...
        InsertCBCustomer objDML = new InsertCBCustomer(connection);
        try
        {
            objDML.CustomersCId = this.txtDescription.Text;
            objDML.CustomersDesc ="";
            objDML.Amount =Convert.ToDecimal(this.txtFee.Text);
            objDML.SPAutoId =Convert.ToInt32(this.ddlSalesPerson.SelectedItem.Value.ToString());
            objDML.PCAutoId = Convert.ToInt32(this.ddlPriceClass.SelectedItem.Value.ToString());
            objDML.Terms = this.txtTerms.Text;
    ...
    protected void mUpdate()
    {
        ...
        SqlConnection connection = new SqlConnection(cnnString);
        UpdateCBCustomer objDML = new UpdateCBCustomer(connection);
        try
        {
            objDML.CustomersId= Int32.Parse(this.HidCID.Value.ToString());
            objDML.CustomersCId = this.txtDescription.Text;
            objDML.CustomersDesc = "";
            objDML.Amount = Convert.ToDecimal(this.txtFee.Text);
            objDML.SPAutoId = Convert.ToInt32(this.ddlSalesPerson.SelectedItem.Value.ToString());
    ...
    protected void cmdDelete_Click(object sender, EventArgs e)
    {
        ...
        {
            myScript = "showInfo('Please select a Customer to delete.');";
            ScriptManager.RegisterStartupScript(Page, this.GetType(), "ClientScript", myScript, true);
            myScript = "";
            return;
        }
        ...
        strSQL = @"SELECT COUNT(CustomerId) CVAL FROM TB_PrmUserInfo WHERE CustomerId IN (" + completeString + ")";
        ...
        strSQL = @"DELETE FROM [dbo].[CB_Customers] WHERE CustomersId IN (" + completeString + "); ";
        object obj = null;
